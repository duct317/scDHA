...
scDHA can be installed fron GitHub using below instruction. scDHA depends on the `torch` package to build and train the autoencoders. The first time `scDHA` packaged is loaded, `torch` package will install the necessary C++ `libtorch`, which is used for neural network computation. 
```{r, eval=FALSE}
#Install devtools: 
...
#The first time the package is loaded, torch package will install C++ libtorch 
library(scDHA)
```
